...
ros::Publisher visImagePublisher;
              ...
              const autoware_msgs::Signals::ConstPtr& signalMsg)
{
    ...
    autoware_msgs::ImageRect info_msg;
    ...
    if(roiImageArray.objects.size() > 0){
        ...
        if(debug_){
            sensor_msgs::ImagePtr visImageMsg =
                cv_bridge::CvImage(inImageMsg->header, "rgb8", vis_image).toImageMsg();
            visImagePublisher.publish(visImageMsg);
        }
    }
    infoPublisher.publish(info_msg);
...
int main(int argc, char *argv[])
{
    ...
    maskImagePublisher =
        rosnode.advertise<sensor_msgs::Image>("/feat_proj_bbox/output", 100);
    nearestRoiImagePublisher =
        rosnode.advertise<sensor_msgs::Image>("/feat_proj_bbox/nearest_box", 100);
    visImagePublisher =
        rosnode.advertise<sensor_msgs::Image>("/feat_proj_bbox/vis_image", 100);
    roiImageArrayPublisher =
        rosnode.advertise<autoware_msgs::DetectedObjectArray>("/feat_proj_bbox/output/roi_image_array", 100);
    infoPublisher =
        rosnode.advertise<autoware_msgs::ImageRect>("/feat_proj_bbox/nearest_info", 100);
